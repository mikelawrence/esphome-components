...
InstallationAngleNumber = ld2450_ns.class_("InstallationAngleNumber", number.Number)
...
async def to_code(config):
    ...
    if installation_angle_config := config.get(CONF_INSTALLATION_ANGLE):
        n = await number.new_number(
            installation_angle_config,
            min_value=-45,
            max_value=45,
            step=1,
        )
        await cg.register_parented(n, config[CONF_LD2450_ID])
        cg.add(ld2450_component.set_installation_angle_number(n))
    for zone_num in range(MAX_ZONES):
        if zone_conf := config.get(f"zone_{zone_num + 1}"):
            ...
            await number.register_number(
                x1, zone_x1_config, min_value=-6000, max_value=6000, step=1
            )
            ...
            await number.register_number(
                y1, zone_y1_config, min_value=-1560, max_value=6000, step=1
            )
            ...
            await number.register_number(
                x2, zone_x2_config, min_value=-6000, max_value=6000, step=1
            )
            ...
            await number.register_number(
                y2, zone_y2_config, min_value=-1560, max_value=6000, step=1
            )
