  ...
  namespace dfrobot_c4001
  {
        ...
        else if (message.starts_with("$DFDMD"))
        {
          ...
          if (target.has_value() && target == 1)
          {
            ...
            if (target_distance.has_value() && target_speed.has_value() && target_energy.has_value())
            {
              ...
              this->parent_->set_occupancy(true);
              this->parent_->set_enable(true);
              ESP_LOGV(TAG, "Recv Rpt: Target Detected, Dist=%.3f, Speed=%.3f, Energy=%d",
                       target_distance.value(), target_speed.value(), (uint)target_energy.value());
              return 1; // command completed successfully
            }
            else
            {
              ESP_LOGE(TAG, "Error parsing Distance and Speed Detection Output (%s)", message.c_str());
    ...
    uint8_t GetRangeCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Range: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Range complete: Parsed Min Range (%.3f) and Max Range (%.3f)",
    ...
    uint8_t SetRangeCommand::on_message(std::string &message)
    {
      ...
      {
        ESP_LOGE(TAG, "Cannot configure Range. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetTrigRangeCommand::GetTrigRangeCommand()
    {
      cmd_ = "getTrigRange";
    }
    ...
    uint8_t GetTrigRangeCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Trigger Range: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Trigger Range complete: Parsed Trigger Range Range (%.3f)",
                 this->trigger_range_);
        return 1; // Command done
      }
      else
    ...
    uint8_t SetTrigRangeCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Trigger Range. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetSensitivityCommand::GetSensitivityCommand()
    {
      cmd_ = "getSensitivity";
    }
    ...
    uint8_t GetSensitivityCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Sensitivity: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
    ...
    uint8_t SetSensitivityCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Sensitivity. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetLatencyCommand::GetLatencyCommand()
    {
      cmd_ = "getLatency";
    }
    ...
    uint8_t GetLatencyCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Latency: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Latency complete: Parsed On Latency (%.3f) and Off Latency (%.3f)",
                 this->on_latency_, this->off_latency_);
        return 1; // Command done
      }
      else
      {
    ...
    uint8_t SetLatencyCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Latency. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetInhibitTimeCommand::GetInhibitTimeCommand()
    {
      cmd_ = "getInhibit";
    }
    ...
    uint8_t GetInhibitTimeCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Inhibit Time: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Inhibit Time complete: Parsed Inhibit Time (%.3f)",
                 this->inhibit_time_);
        return 1; // Command done
      }
      else
    ...
    uint8_t SetInhibitTimeCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Inhibit Time. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetThrFactorCommand::GetThrFactorCommand()
    {
      cmd_ = "getThrFactor";
    }
    ...
    uint8_t GetThrFactorCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Threshold Factor: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Threshold Factor complete: Parsed Threshold Factor (%.0f).",
                 this->threshold_factor_);
        return 1; // Command done
      }
      else
      {
    ...
    uint8_t SetThrFactorCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Threshold Factor. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    GetMicroMotionCommand::GetMicroMotionCommand()
    {
      cmd_ = "getMicroMotion";
    }
    ...
    uint8_t GetMicroMotionCommand::on_message(std::string &message)
    {
      ...
      if (message.starts_with("Response"))
      {
        ...
        {
          ESP_LOGE(TAG, "Get Micro Motion: Failed to parse response '%s'.", message.c_str());
          return 1; // Command done
        }
      }
      else if (message == "Done")
      {
        ...
        ESP_LOGV(TAG, "Get Micro Motion complete: Parsed Micro Motion (%s).",
                 this->micro_motion_ ? "Enabled" : "Disabled");
        return 1; // Command done
      }
      else
      {
    ...
    uint8_t SetMicroMotionCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot set Micro Motion. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
    ...
    FactoryResetCommand::FactoryResetCommand()
    {
      cmd_ = "resetCfg";
    }
    ...
    uint8_t SetRunAppCommand::on_message(std::string &message)
    {
      if (message == "sensor is not stopped")
      {
        ESP_LOGE(TAG, "Cannot configure Mode. Sensor is not stopped!");
        return 1; // Command done
      }
      else if (message == "Done")
      {
